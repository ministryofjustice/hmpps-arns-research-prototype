...
const PREDICTORS_JOURNEY_PATH = '/03/'

/** Absolute href for predictors pages (safe from index and /03/* routes) */
...
export const redirectIfPredictorsJourneyIncomplete = () => {
  if (!window.location.pathname.includes('/03/')) return false
...
export const getContinueHrefAfterCheckAnswersEdit = (currentPage, beforeSession, afterSession) => {
  ...
  if (currentPage === 'a3' && afterSession.sexualOffence === 'yes' && !isA3Complete(afterSession)) {
    return getFirstIncompleteA3Page(afterSession) || 'a3.html'
  }
