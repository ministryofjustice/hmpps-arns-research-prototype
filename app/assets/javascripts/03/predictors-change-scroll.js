...
const isProto3PredictorsPage = () => window.location.pathname.includes('/03/')
...
export const startPredictorsCheckAnswersEdit = (fromParam, returnAnchor) => {
  ...
  } else if (window.location.pathname.includes('/03/b11')) {
    updates.checkAnswersReturnTarget = CHECK_ANSWERS_RETURN_B11
  }
...
export const initPredictorsCheckAnswersEditFromUrl = () => {
  if (!isProto3PredictorsPage()) return
...
const toPredictorsHref = (href) => {
  if (!href) return href
  return predictorsJourneyHref(href)
}
...
export const completePredictorsPageAndContinue = (currentPage, defaultHref, newFields) => {
  ...
  if (wasEdit) {
    ...
    if (!changed) {
      ...
      if (wasEdit || snapshot) {
        return toPredictorsHref(resolveReturnHref())
      }

      return toPredictorsHref(nextHref)
    }
    ...
    if (continueHref) {
      const openingDynamic = isOpeningDynamicFromInterviewEdit(currentPage, beforeSession, merged)
      ...
      const resolvedContinueHref = toPredictorsHref(continueHref)
      return openingDynamic ? resolvedContinueHref : withFromCheckAnswers(resolvedContinueHref)
    }
...
window.GOVUKPrototypeKit.documentReady(() => {
  ...
  document.addEventListener('click', (event) => {
    const changeLink = event.target.closest('a.govuk-link')
