...
import { setConditionalVisible } from './tiering-conditional-fields.js'
import {
  getB9BackHref,
  getB9FieldsFromForm,
...
window.GOVUKPrototypeKit.documentReady(() => {
  ...
  const form = document.getElementById('tiering-b9-form')
  if (!form) return
  ...
  const backLink = document.getElementById('tiering-b9-back')
  if (backLink) {
    backLink.href = getTieringBackLinkHref(getB9BackHref())
  }
  ...
  form.addEventListener('submit', (event) => {
    ...
    window.location.href = tieringJourneyHref(
      completeTieringPageAndContinue('b9', getPostB9ContinueHref(), {
        ...newFields,
        b9Complete: true
      })
    )
