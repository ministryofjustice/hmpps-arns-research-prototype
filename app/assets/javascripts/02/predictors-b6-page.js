...
window.GOVUKPrototypeKit.documentReady(() => {
  ...
  const form = document.getElementById('predictors-b6-form')
  if (!form) return
  ...
  const backLink = document.getElementById('predictors-b6-back')
  if (backLink) {
  ...
  form.addEventListener('submit', (event) => {
    ...
    if (!newFields.alcoholFrequencyLast3Months) {
      form.querySelector('#predictors-alcohol-frequency')?.scrollIntoView({ block: 'start' })
    ...
    window.location.href = predictorsJourneyHref(
      completePredictorsPageAndContinue('b6', getPostB6bContinueHref, newFields)
    )
  })
})
